...
class PureRatesValidationTest {
	...
	public void isSmallChangeTest() {
		...
		ExchangeRateSet smallChangeDecrease =
				newRates(midnightRates, true, true, false);
		Assertions.assertTrue(isNormalIntradayChange(midnightRates, smallChangeDecrease, bound));

		ExchangeRateSet bigChangeInCurrent =
				newRates(midnightRates, false, true, false);
		Assertions.assertFalse(isNormalIntradayChange(midnightRates, bigChangeInCurrent, bound));

		ExchangeRateSet increasingBigChangeInCurrent =
		...
		ExchangeRateSet bigChangeInNext =
				newRates(midnightRates, true, false, true);
		Assertions.assertFalse(isNormalIntradayChange(midnightRates, bigChangeInNext, bound));

		ExchangeRateSet decreasingBigChangeInCurrent =
		...
		ExchangeRateSet bigChangeInBoth =
				newRates(midnightRates, false, false, true);
		Assertions.assertFalse(isNormalIntradayChange(midnightRates, bigChangeInBoth, bound));

		ExchangeRateSet decreasingBigChangeInBoth =
	 ...
	 * values in the given oldC and oldH A change is a small change iff for arbitrary-precision numbers:
	 * <pre>
	 ...
	 * </pre>
	 */
	private Pair<Integer, Integer> getNewHandC(int oldH, int oldC, boolean smallChange, boolean increase) {
		int newH = oldH;
