...
public class TopicTest {
	...
	public void constructorWithNullsTestingAccessors() {
		// given:
		var topic = new Topic(null, null, null, 0L, null, null);
	...
	public void unimplementedMethodsThrowUnsupported() {
		...
		// expect:
		assertThrows(UnsupportedOperationException.class, () -> topic.copyFrom(null));
		assertThrows(UnsupportedOperationException.class, () -> topic.copyFromExtra(null));
		assertThrows(UnsupportedOperationException.class, () -> topic.diffCopyTo(null, null));
		assertThrows(UnsupportedOperationException.class, () -> topic.diffCopyFrom(null, null));
	}

	@Test
	public void equalsDefault() {
		// expect:
		assertTrue(new Topic().equals(new Topic()));
	}

	@Test
	public void equalsNull() {
		// expect:
		assertFalse(new Topic().equals(null));
	}

	@Test
	public void equalsWrongClass() {
		// expect:
		assertFalse(new Topic().equals(""));
	}
	...
	public void equalsViaCopy(String memo, @ConvertWith(JEd25519KeyConverter.class) JEd25519Key adminKey,
		...
		// expect:
		assertTrue(topic.equals(new Topic(topic)));
	}
	...
	public void notEquals(String aMemo, @ConvertWith(JEd25519KeyConverter.class) JEd25519Key aAdminKey,
		...
		// given:
		var a = new Topic(aMemo, aAdminKey, aSubmitKey, aAutoRenewDurationSeconds, aAutoRenewAccountId,
				aExpirationTimestamp);
		a.setDeleted(aDeleted);
		a.setSequenceNumber(aSequenceNumber);
		a.setRunningHash(aRunningHash);
		var b = new Topic(bMemo, bAdminKey, bSubmitKey, bAutoRenewDurationSeconds, bAutoRenewAccountId,
	...
	public void hashIsSafeOnDefault() {
		// expect:
		assertDoesNotThrow(() -> new Topic().hashCode());
	}
	 ...
	 * @param topic
	 */
	private void assertDefaultTopicAccessors(Topic topic) {
	...
	public void assertPostConstructorAccessors(Topic topic, String memo, JKey adminKey, JKey submitKey,
		...
		assertArrayEquals(adminKey.serialize(), topic.getAdminKey().serialize());
		assertTrue(topic.hasSubmitKey());
		assertArrayEquals(submitKey.serialize(), topic.getSubmitKey().serialize());
